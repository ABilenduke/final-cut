...
- **[modify]** `backend/database/migrations/2026_04_04_200009_create_bookings_table.php` — Add the `promo_code_id` BIGINT column + `index(['promo_code_id', 'status'])`. **SHIPPED WITHOUT `->constrained()->nullOnDelete()` here** (deviations note #1): `bookings` is created BEFORE `promo_codes`, so the FK constraint is added in the `create_promo_codes_table` migration instead. foreignId NOT foreignUuid (promo_codes uses $table->id()). Edit in place; `make fresh` after.
...
- **[modify]** `backend/app/Http/Controllers/Api/BookingController.php` — Add `use App\Services\PromoRedemptionIdentity;` (alphabetical among App\Services). Build identity for validateCode() pre-check (line 117). Set `$booking->promo_code_id = $promo?->id;` before save in store() Phase C (line 374) and confirm() Phase C (line 595). Extend finalizeBooking() signature with `?PromoRedemptionIdentity $promoIdentity = null` and pass it to consume(). Build identity at both finalize call sites (store line 376 from $booking + authed account email; confirm line 599 from $pendingData). *(The dossier's step (5) — a per-user pre-charge check in confirm() Phase A — was **NOT shipped**; see deviations note #2. The `consume()` lock is the authoritative backstop, charge-then-refund 409, for the rare concurrent-3DS race.)*
