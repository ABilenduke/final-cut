...
- **A1-A3** — enum-validate API filters → 422: `?status` (MovieController, validator placed BEFORE location resolution so a bad status isn't masked by a bad-location 422), `?type`/`?accessibility` (CalendarEventController; accessibility is a closure allowlist that skips empty CSV segments to keep trailing-comma tolerance), `?category` (FoodMenuController). **`?per_page` is CLAMPED, not validated** — see the e2e regression note below.
...
- **E2E `up --wait` regression:** the new worker healthcheck made `docker compose up --wait` gate on `backend-worker` liveness, which isn't continuously healthy under the throwaway e2e config → `container backend-worker is unhealthy`. A first attempt (`healthcheck: disable: true`) then failed differently — `--wait` errors with `container … has no healthcheck configured` on a *disabled* healthcheck. Final fix: set `backend-worker` + `backend-scheduler` to `deploy.replicas: 0` in `docker-compose.e2e.yml` (same pattern as fail2ban) — the Playwright e2e never depends on background-queue/scheduler work, and the original e2e passed with them running unhealthchecked. Production healthchecks unchanged. Copilot's "`pgrep` absent (no `procps`)" was incorrect — it's BusyBox-provided and verified working in a `--target production` build.
...
- **E2E sitemap regression (A1 contract change):** the dossier's A1 added a `per_page max:100` validator that 422'd large values. `frontend/server/api/__sitemap__/urls.get.ts` fetches `/api/movies?per_page=500` to source dynamic sitemap URLs, so the 422 dropped every movie URL → 6 `sitemap.spec.ts` e2e failures. (The dossier's pre-flight grep checked `frontend/app/` but not `frontend/server/`.) Fix per the dossier's own fallback: validate ONLY `?status`; `?per_page` stays **clamped** to 100 (the long-standing contract), never 422'd.
