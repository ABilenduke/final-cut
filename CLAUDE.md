...
- **Dispatch outbox** ships now: `App\Outbox\OutboxDispatcher` maps `event_type` → job, `outbox:dispatch` runs every minute (`withoutOverlapping(2)` + `runInBackground()`), `outbox:prune` runs daily (14-day retention aligned with activity_log). `backend-worker` and `backend-scheduler` compose services reuse the backend image — no separate Dockerfile. Both carry a process-liveness **healthcheck** (`pgrep -f '[q]ueue:work'` / `'[s]chedule:work'`; `pgrep` is BusyBox-provided in the php-alpine base — no `procps`; the `[q]`/`[s]` bracket prevents the healthcheck shell self-matching). The healthchecks are **disabled in the e2e overlay** (`docker-compose.e2e.yml` sets both to `replicas: 0`, like fail2ban) because the Playwright suite doesn't exercise background work and a disabled-healthcheck or unhealthy worker would fail `docker compose up --wait`.
...
- **`booking_seats` occupancy guard** (post-P1 hardening, migration `2026_06_05_000000_add_booking_seats_occupancy_guard`): the seat-map analogue of the `showtimes_no_overlap` EXCLUDE constraint. A denormalized `booking_seats.occupies_seat` boolean (kept in sync with the parent booking's status by two Postgres triggers — BEFORE INSERT/UPDATE OF `booking_id`, and AFTER UPDATE OF `status` on `bookings` for event-bypassing writes) backs a partial `UNIQUE INDEX booking_seats_one_occupant_per_seat (showtime_id, seat_id) WHERE occupies_seat AND seat_id IS NOT NULL`. This is the authoritative DB-level guarantee that two occupying bookings (confirmed/held/refund_pending) can never hold the same `(showtime, seat)`; `SeatAvailabilityService::reserveSeats` translates the index's 23505 into `SeatConflictException` (409). The SQL helper `fc_status_occupies()` MUST stay in lockstep with `BookingStatus::occupyingStatuses()` (a parity test fails CI on drift). This is the one documented **additive** exception to the pre-launch edit-in-place rule.
...
- `NUXT_PUBLIC_SITE_URL` is the public base URL used for SEO/OG tags, the sitemap, and emails (maps to `runtimeConfig.public.siteUrl`, default `https://finalcut.test`). `NUXT_PUBLIC_APP_TIME_ZONE` is the IANA timezone for SSR-safe date-only UI (e.g. the `/whats-on` "today" key; `runtimeConfig.public.appTimeZone`, default `America/New_York`). Both are in `frontend/.env.example`.
...
**Boot cache contract:** the backend `dev-entrypoint.sh` runs `php artisan optimize:clear` then `php artisan optimize` on every container boot — flushing and rebuilding the bootstrap/config/route/view caches and any stale Redis-backed application cache (food menu, featured slides) so a `make down/build/up` never carries stale state. `make fresh` mirrors this (clear + rebuild after `migrate:fresh --seed`) so it leaves the same warmed-cache state as a fresh boot. **Tradeoff:** because `optimize` runs `config:cache`, edits to `backend/.env` don't take effect until the next container restart or a manual `php artisan optimize:clear`.
...
- **Rendering**: `routeRules` in `nuxt.config.ts` control per-route rendering strategy — ISR for blog (`/blog/**`) and for the admin-managed content pages (`/contact`, `/faq`, `/accessibility`, `/careers`, `/private-screenings` — flipped from prerender as their content moved behind APIs); prerender remains for `/gift-cards/bulk`. See @docs/architecture/SITE_ARCHITECTURE.md for the full route map
...
- **Backend test helpers** (in `backend/tests/Helpers/`): `BookingTestHelper` trait (`createShowtimeWithSeats()` → `['location','auditorium','seats','showtime','movie']`; `bookingUrl($location[, 'confirm'])`; `fakeStripe()` binds `FakeStripeService`) and `AdminAuthHelper` trait (`actingAsAdmin()`, auto-applied to `Feature/Admin` + `Unit/Admin`). `FakeStripeService` drives payment outcomes via `shouldSucceed()` / `shouldRequire3ds()` / `shouldDecline()` / `shouldFailWith*()` and records `confirmedPaymentIntents` / `refundedPaymentIntents`. Customer auth in tests uses Pest's `actingAs($user)` directly (there is no `AuthHelper`/`StripeHelper` trait).
