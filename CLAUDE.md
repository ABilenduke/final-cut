...
- **Dispatch outbox** ships now: `App\Outbox\OutboxDispatcher` maps `event_type` → job, `outbox:dispatch` runs every minute (`withoutOverlapping(90)` + `runInBackground()`), `outbox:prune` runs daily (14-day retention aligned with activity_log). `backend-worker` and `backend-scheduler` compose services reuse the backend image — no separate Dockerfile.
